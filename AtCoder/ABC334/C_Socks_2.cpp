...
ll get_odd(ll n, vll &v) {
	vector<vll> cnt(2, vll(n + 1, 0));
	rep (i, 0, n-1) {
		if (i % 2 == 0) {
	...
	vector<vll> pre(2, vll(n + 1, 0));
	rep (i, 0, n) {
		// rep (j, 0, 2) {
		// 	pre[j][i + 1] = cnt[j][i] + pre[j][i];
		// }
		pre[0][i + 1] = cnt[0][i] + pre[0][i];
		// pre[1][n - 1 - i] = cnt[1][n - 1 - i] + pre[1][n - i];
	}

	for (ll j = n; j > 0; j -=1) {
		pre[1][j - 1] = cnt[1][j-1] + pre[1][j];
	}
	showv(v);	// debug
	showmat(cnt);	// debug
	showmat(pre);	// debug
	ll ans = 1e12;
	por (i, 0, n, 2) {
		// ll num =  pre[0][i] + (pre[1][n-1] - pre[1][i]);
		ll num =  pre[0][i] +  pre[1][i+1];
		ans = min(ans, num);
	}
	return ans;
}
