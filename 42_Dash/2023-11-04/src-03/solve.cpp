...
ll bfs(vector<vector<ll>> &cost, vector<string> &keys, int N, int M, pair<int, int> start, pair<int, int> end, map<char, int> &key_map, string &ans_path) {
    ll ans = 1e9;
    ...
    while (!q.empty()) {
        ...
        for (int id = 0; id < 4; id++) {
            ...
            int val = mat[p[1]][p[2]] + 2 * (int)(fabs(x - end.first) + fabs(y - end.second))
                        + cost[x][y] * (7 + !key_map[keys[x][y]] - key_map[keys[x][y]]);
            if (mat[x][y] > val) {
                mat[x][y] = val;
...
void	solve(vector<string> &grid, int N, int M) {
    ...
    for (int i = 0; i < 21; i++) {
        ...
        key_map['A'] = combi[i][1];
        key_map['E'] = combi[i][2];
        ll val = bfs(cost, keys, N, M, start, end, key_map, path);
        // for (int j = 0; j < 3; j++) {
        //     cout << combi[i][j] << " ";
        // }
        // cout << ": " << val << endl;
