...
void DFS(vvi &maze, pii start, vint need, vvi &vis, string &str);
...
int main () 
{
	...
	do {
		...
		seti emp;
		comb.pb(emp);
		DFS(maze, sta, need, vis, str);
	} while (next_permutation(str.begin(), str.begin() + 4));
...
void set_maze(vstr &lines, vvi &maze, pii &start) {	
	...
	dir.insert(mk('A', mk(-1, 0)));
	dir.insert(mk('B', mk(0, 1)));
	dir.insert(mk('C', mk(0, -1)));
	dir.insert(mk('D', mk(1, 0)));
...
void DFS(vvi &maze, pii start, vint need, vvi &vis, string &str) {
	int r = start.ff;
	int c = start.ss;
	if (vis[r][c] == 1 || r < 0 || c < 0 || r >= n || c >= n) return;
	...
	rep (i, 0, 4) {
		char ch = str[i];
		AA = dir.find(ch);
		pii pos = mk(r + AA->ss.ff, c + AA->ss.ss);
		DFS(maze, pos, need, vis, str);
	}
}
