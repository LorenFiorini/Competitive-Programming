...
bool add (int num, vint &need) {
	...
	bool push = true;
	rep (i, 0, len(need)) {
		pii ppp = mk( min(num, need[i]), max(num, need[i]));
		if ( ! notin(imp , ppp)) push = false;
	}
	if (push) need.pb(num);
	
	return count_valid_combinations(need);
}
/*
bool fun (vint &need) {
...
void DFS(vvi &maze, pii start, vint need, vvi &vis) {
	...
	if (num == -1) return;
		...
		// true if creat a valid table 
		if (add(num, need)) return;
	}
	...
	rep (i, 0, len(comb))
	{
		...
		if (MOO(tictac)) {
			if (!valido) {
				bool ok = true;
				rep (k, 0, len(need)) {
					if (notin(comb[i] ,need[k])) ok = false; 
				}
				if (ok && len(need) == len(comb[i])) valido = true;
			}
